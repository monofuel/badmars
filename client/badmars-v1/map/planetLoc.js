...
export class PlanetLoc {
  ...
  constructor(planet: Map, x: number, y: number) {
      ...
      // loop around bottom
      if (this.x < 0) {
        this.x = (this.planet.settings.size + this.x - 1) %
          (this.planet.settings.size - 1);
      }
      // loop around the top
      ...
      if (this.y < 0) {
        this.y = (this.planet.settings.size + this.y - 1) %
          (this.planet.settings.size - 1);
      }
  ...
  toString(): string {
    ...
    if (this.planet && this.planet.settings) {
      line += ", planet: " + this.planet.settings.name;
    }
    return line;
  }
