...
func (c *BMClient) Register(username string, email string, password string) (*User, error) {
	...
	if resp.StatusCode != 200 {
		return nil, fmt.Errorf("bad registration response %v", resp)
	}
	...
	c.SessionToken = registration.SessionToken
	...
	selfReq.Header.Set("Authorization", "Bearer "+c.SessionToken)
	...
	if selfResp.StatusCode != 200 {
		return nil, fmt.Errorf("bad status code for self: %v", selfResp.StatusCode)
	}
