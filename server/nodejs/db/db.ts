...
export default class DB {
  ...
  async connect(): Promise<void> {
    const options: {
      host: string,
      db: string, port?: number, user?: string, password?: string
    } = {
        host: env.dbHost,
        db: env.database,
      };
    ...
    try {
      ...
      logger.trackError(
        null, new WrappedError(
          err, 'failed to connect to DB, retrying in 5 seconds'));
      await sleep(5000);
      return this.init();
    }
  }
  ...
  async setupSchema(): Promise<void> {
    ...
    await Promise.all(
      mapNames.map(
        (name: string): Promise<void> => setupPlanet(this.conn, name)));

    // TODO
    logger.info(null, 'Schema Initialized');
  }

  async close(): Promise<void> { return this.conn.close(); }
}
