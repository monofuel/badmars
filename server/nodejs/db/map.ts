...
export default class DBMap {
  ...
  async listNames(): Promise<Array<string>> {
    return await (this.table as any)
      .getField('name')
      .coerceTo('array')
      .run(this.conn);
  }
  ...
  async getMap(ctx: Context, name: string, opts?: any): Promise<Map> {
    ...
    if (!ignoreCache &&
      this.mapCache
      [name] /*&& Date.now() - mapCache[name].lastUpdate < 2000*/) {
    ...
    const map = new Map();
    map.clone(doc);
    this.mapCache[name] = { lastUpdate: Date.now(), map: map };
  ...
  async registerListener(name: string, func: Function): Promise<void> {
    this.table.get(name).changes().run(this.conn).then(
