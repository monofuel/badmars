...
export default class Planet implements DB.Planet {
  ...
  public chunk: DBChunk;
  public chunkLayer: DBChunkLayer;
  public unit: DBUnit;
  public factoryQueue: DBFactoryQueue;
  public unitStat: DBUnitStat;
  ...
  public async patch(ctx: Context, patch: Partial<GamePlanet>): Promise<void> {
    ...
    const result = await this.table.get(this.planet.name).update(patch,
      { returnChanges: 'always' as any }).run(this.conn);
    await call.end();
