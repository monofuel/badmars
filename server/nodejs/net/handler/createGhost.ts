...
export default async function createGhost(
  ...
  try {
    ...
    unit.details.ghosting = true; unit.details.owner = client.user.uuid;

    await map.spawnUnit(ctx, unit);

    await client.send('createGhost');

    // wake up nearby ghost builders
    /*
    // TODO
    const units: Array<Unit> = await map.getNearbyUnitsFromChunk(ctx,
    unit.location.chunkHash[0]);
