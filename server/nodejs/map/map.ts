...
const mapClient = new services.Map(env.mapHost + ':' + env.mapPort,
grpc.credentials.createInsecure());
*/

const defaultSettings = {
  chunkSize: Number(process.env.CHUNK_SIZE) || 16,
  waterHeight: 0,
  cliffDelta: 0.7,
  water: true,
  bigNoise: 0.005,
...
function containsTile(list: PlanetLoc[], tile: PlanetLoc): boolean {
...
interface CacheChunkType {
  chunk: Chunk;
  timestamp: number;
}
interface ChunkCacheMapType {
...
export default class Map {
  ...
  public async getChunkOld(ctx: Context, x: number, y: number): Promise<Chunk> {
    const planetDB = await db.getPlanetDB(ctx, this.name);
  ...
  public async getLoc(ctx: Context, x: number, y: number): Promise<PlanetLoc> {
    const planetDB = await db.getPlanetDB(ctx, this.name);
    ctx.check('getLoc');
    const details = getLocationDetails(x, y, this.settings.chunkSize);

    const chunk: Chunk =
      await this.getChunkOld(ctx, details.chunkX, details.chunkY);
  ...
  public async factoryMakeUnit(
    ctx: Context, unitType: string, owner: string, x: number,
    y: number): Promise<null | Unit> {
  ...
  public async spawnUnit(ctx: Context, newUnit: Unit): Promise<Unit> {
    ctx.check('spawnUnit');
  ...
  public async spawnUnitWithoutTileCheck(ctx: Context, newUnit: Unit): Promise<Unit> {
    const planetDB = await db.getPlanetDB(ctx, this.name);
  ...
  public async spawnAndValidate(ctx: Context, newUnit: Unit): Promise<Unit> {
    ctx.check('spawnAndValidate');
    const planetDB = await db.getPlanetDB(ctx, this.name);
    const unit: Unit = await planetDB.unit.create(ctx, newUnit);
    try {
      await await addToChunks(ctx, unit);
    } catch (err) {
      try {
        await planetDB.unit.delete(ctx, unit.uuid);
      } catch (err) {
        throw new WrappedError(
          err, 'failed to cleanup with failed spawn in spawnAndValidate');
      }
  ...
  public async checkValidForUnit(
    ctx: Context, tiles: PlanetLoc[], unit: Unit,
    ignoreMoving: boolean = false): Promise<null | string> {
    ...
    for (const tile of tiles) {
      const units2: Unit[] =
        await this.unitsTileCheck(ctx, tile, unit.details.ghosting);
  ...
  public async checkOpen(ctx: Context, tile: PlanetLoc): Promise<boolean> {
    ctx.check('checkOpen');
    // TODO handle air units
    return (await this.unitsTileCheck(ctx, tile, false)).length === 0;
  }

  public async unitsTileCheck(
    ctx: Context, tile: PlanetLoc,
    includeGhosts: boolean = false): Promise<Unit[]> {
    // const units = await tile.chunk.getUnits(tile.hash);
    const units: Unit[] = await tile.getUnits(ctx);
    if (!includeGhosts) {
      return _.filter(
        units, (unit: Unit): boolean => !unit.details.ghosting);
    } else {
      return units;
    }
  }

  public async spawnUser(ctx: Context, user: User): Promise<void> {
    const planetDB = await db.getPlanetDB(ctx, this.name);
    ...
    const unitsToSpawn: string[] = [
      ...
      'transfer_tower', 'factory', 'transport', 'scout',
    ];
          ...
          for (const unit of spawnedUnits) {
            try {
              ...
              logger.trackError(
                ctx,
                new WrappedError(
                  err, 'failed to remove unit during rollback unit spawn'));
            }
            try {
              await planetDB.unit.delete(ctx, unit.uuid);
            } catch (err) {
              logger.trackError(
                ctx,
                new WrappedError(
                  err, 'failed to delete unit to rollback unit spawn'));
            }
          }
          throw new WrappedError(
            err, 'spawning unit failed, bailing out', { x, y, unitType });
        }
      }
      for (const unit of spawnedUnits) {
        await planetDB.unit.patch(ctx, unit.uuid, unit);
      }
    }
  ...
  // find random spawn locations, looking farther away depending on how many
  // attempts tried
  // returns a chunk of all free tiles
  public async findSpawnLocation(ctx: Context, attempts: number = 0): Promise<Chunk> {
    attempts = attempts || 0;
  ...
  public async getNearbyUnitsFromChunkWithTileRange(
    ctx: Context, chunkHash: ChunkHash,
    tileRange: number): Promise<Unit[]> {
  ...
  public async getUnitsAtChunks(ctx: Context, chunkHashes: ChunkHash[]):
    Promise<Unit[]> {
  ...
  public getNearbyChunkHashes(chunkHash: string, range: number): ChunkHash[] {
    const x = Number(chunkHash.split(':')[0]);
    const y = Number(chunkHash.split(':')[1]);
  ...
  public async validChunkForSpawn(ctx: Context, chunk: Chunk): Promise<boolean> {
    const nearbyUnits =
      await this.getNearbyUnitsFromChunkWithTileRange(ctx, chunk.hash, 32);
    ...
    const tiles: PlanetLoc[] = await planetLocsForChunk(ctx, chunk);
  ...
  public async pullResource(ctx: Context, type: Resource, taker: Unit, amount: number):
    Promise<boolean> {
    const planetDB = await db.getPlanetDB(ctx, this.name);

    // TODO should calculate based on transfer range
    let units: Unit[] =
      await this.getNearbyUnitsFromChunk(ctx, taker.location.chunkHash[0]);
    ...
    for (const unit of units) {
      ...
      const pulled =
        await planetDB.unit.pullResource(ctx, type, amount, unit.uuid);
      if (pulled > 0) {
  ...
  public async produceResource(
    ctx: Context, type: Resource, mine: Unit, amount: number): Promise<void> {
    const planetDB = await db.getPlanetDB(ctx, mine.location.map);

    // TODO should calculate based on transfer range
    const units: Unit[] =
      await this.getNearbyUnitsFromChunk(ctx, mine.location.chunkHash[0]);
    ...
    for (const unit of units) {
      ...
      const deposited: number =
        await planetDB.unit.putResource(ctx, type, amount, unit.uuid);
      ...
      if (amount > 0) {
        const deposited: number =
          await planetDB.unit.putResource(ctx, type, amount, mine.uuid);
      }
    }
  }

  public sortByNearestUnit(units: Unit[], unit: Unit) {
    units.sort((a: Unit, b: Unit): number => {
      return unitDistance(a, unit) - unitDistance(b, unit);
    });
  }

  public sortBuildingsOverOther(units: Unit[]) {
    units.sort((a: Unit, b: Unit): number => {
  ...
  public async getNearestFreeTile(
    ctx: Context, center: PlanetLoc, unit?: Unit,
    includeGhosts?: boolean): Promise<null | PlanetLoc> {
    ctx.check('getNearestFreeTile');
    const unitsOnTile: Unit[] =
      await this.unitsTileCheck(ctx, center, includeGhosts);
    ...
    while (true) {
      ...
      open.sort(
        (a: PlanetLoc, b: PlanetLoc): number => a.cost - b.cost);
      for (const openTile of open) {
        if (containsTile(closed, openTile)) {
          continue;
        }
        const unitsOnTile =
          await this.unitsTileCheck(ctx, openTile, includeGhosts);
      ...
      for (const tile of open) {
        const neighbors = [
          await tile.N(ctx), await tile.S(ctx), await tile.E(ctx),
          await tile.W(ctx),
        ];
  ...
  public async advanceTick(ctx: Context): Promise<void> {
    return this.update(
      ctx, { lastTickTimestamp: Date.now(), lastTick: this.lastTick + 1 });
  }

  public async setPaused(ctx: Context, paused: boolean): Promise<void> {
    return this.update(ctx, { paused });
  }

  public async update(ctx: Context, patch: object): Promise<void> {
    const planetDB = await db.getPlanetDB(ctx, this.name);
