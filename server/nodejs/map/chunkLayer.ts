...
/*
  ...
  async moveUnit(ctx: Context, unit: Unit, newTile: PlanetLoc): Promise<void> {
    ...
    if (unit.uuid !== newChunk.units[newTile.hash]) {
      throw new DetailedError('wrong new position',
        { hash: newTile.hash, uuid: unit.uuid, otherUuid: newChunk.units[newTile.hash] });
    }
  ...
  async addResource(ctx: Context, uuid: UUID, tileHash: TileHash): Promise<void> {
    ...
    if (uuid !== this.resources[tileHash]) {
      throw new DetailedError('failed to add resource after refresh',
       { uuid, tileHash, found: this.resources[tileHash] });
    }
  }
